...
contract ZMT is IERC20, IZMT, ITimelockTarget {
    using SafeMath for uint256;

    string public constant name = "ZMT";
    string public constant symbol = "ZMT";
    ...
    modifier onlyAdmin() {
        require(admins[msg.sender], "ZMT: forbidden");
        _;
    }
    ...
    function setNextMigrationTime(uint256 _migrationTime) external onlyGov {
        require(_migrationTime > migrationTime, "ZMT: invalid _migrationTime");
        migrationTime = _migrationTime;
    }

    function beginMigration() external override onlyAdmin {
        require(block.timestamp > migrationTime, "ZMT: migrationTime not yet passed");
    ...
    function transferFrom(address _sender, address _recipient, uint256 _amount) external override returns (bool) {
        uint256 nextAllowance = allowances[_sender][msg.sender].sub(_amount, "ZMT: transfer amount exceeds allowance");
    ...
    function _transfer(address _sender, address _recipient, uint256 _amount) private {
        require(_sender != address(0), "ZMT: transfer from the zero address");
        require(_recipient != address(0), "ZMT: transfer to the zero address");

        if (hasActiveMigration) {
            require(allowedMsgSenders[msg.sender], "ZMT: forbidden msg.sender");
            require(!blockedRecipients[_recipient], "ZMT: forbidden recipient");
        }
    ...
    function _mint(address _account, uint256 _amount) private {
        require(_account != address(0), "ZMT: mint to the zero address");
    ...
    function _approve(address _owner, address _spender, uint256 _amount) private {
        require(_owner != address(0), "ZMT: approve from the zero address");
        require(_spender != address(0), "ZMT: approve to the zero address");
