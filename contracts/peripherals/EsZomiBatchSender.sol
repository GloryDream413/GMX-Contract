...
contract EsZomiBatchSender {
    using SafeMath for uint256;

    address public admin;
    address public esZomi;

    constructor(address _esZomi) public {
        admin = msg.sender;
        esZomi = _esZomi;
    }

    modifier onlyAdmin() {
        require(msg.sender == admin, "EsZomiBatchSender: forbidden");
        _;
    }
    ...
    function send(
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
            IERC20(esZomi).transferFrom(msg.sender, _accounts[i], _amounts[i]);
